...
def RRT_plan(map,start,goal):
    ...
    def valid(pin):
        ...
        if(map[pin[0]][pin[1]]==0):
            return(True)
        return(False)
    path=rrt(bounds,valid,start,goal,1000,np.linalg.norm(bounds/10.))
    return(path)

def plot_course(map,start,goal):
    node_at=RRT_plan(map,start,goal)
    #for i in node_at:
...
while robot.step(timestep) != -1:
    if(state=="menu"):
        ...
        if key == ord('L'):  #load previous map
            ...
            plt.show()
            set_pose_via_truth()
            plot_course(map_blocks,we_to_map((pose_x,pose_y)),we_to_map((10,1.8)))
